...
- **[src/App.tsx](src/App.tsx) is the router shell** (react-router v7): Aurora, Navbar, `<Routes>`, Footer, ChatAgent. Routes are `/`, `/services/:slug`, `/privacy`, `/terms`, `*`. Page content lives in [src/routes/](src/routes/); [HomePage](src/routes/HomePage.tsx) composes the one-page sections in fixed order.
- **Nav links are `/#section`, not `#section`**, so they work from a sub-page. `HomePage` scrolls to `location.hash` after paint. Netlify needs the `/* -> /index.html 200` redirect in netlify.toml or deep links 404.
- **Testimonials are commented out** in `HomePage`, pending client-approved quotes and real photos. The component and its data still exist.
- **Motion is framer-motion**, with shared variants in [src/motion/presets.ts](src/motion/presets.ts). Use `revealItem`/`revealStagger` with `whileInView` + `viewportOnce`. Entrance animation belongs there now; CSS keyframes are only for decorative loops (rings, marquee, aurora drift).
- **Kris emails Formspree directly** ([ChatAgent](src/components/ChatAgent/ChatAgent.tsx)); he no longer pre-fills the contact form, and the old `src/enquiry/` hand-off is deleted. The flow asks one field per turn and draws a random couple of follow-up questions from the pools in [script.ts](src/components/ChatAgent/script.ts).
